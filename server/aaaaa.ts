(async function () {
	...
	async function main() {
		async function ensure_dir_exit() {
			const [dirs, files] = await _n.asyncAllDirAndFile([_n.pathR("/media/shone/Elements/resource/Course")]);
			let dir;
			while (dir = dirs.pop()) {
				console.log(dir)
				dir = dir.split("/")[1]
			...
			fs.writeFileSync("./records.course.js", `module.exports=${JSON.stringify([files, cacheMap])}`);
		};
