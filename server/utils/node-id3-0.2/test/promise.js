...
describe("NodeID3.Promise", function () {
	...
	describe("#create()", function () {
		it("resolve", function () {
			return expect(NodeID3.Promise.create({})).to.eventually.be.instanceOf(Buffer);
		});
	});
	...
	writeTestCases.forEach(([name, fn]) => {
		describe(name, function () {
			describe("with buffer", function () {
				it("resolve", function () {
					return expect(fn({}, Buffer.alloc(0))).to.eventually.be.instanceOf(Buffer);
				});
			});
			describe("with invalid file path", function () {
				it("reject", function () {
					return expect(fn({}, invalidFilepath)).to.eventually.be.rejectedWith(Error);
				});
			});
	...
	describe("#read()", function () {
		describe("with buffer", function () {
			it("resolve", function () {
				return expect(NodeID3.Promise.read(Buffer.alloc(0))).to.eventually.to.have.key(
					"raw"
				);
			});
		});
		describe("with invalid file path", function () {
			it("reject", function () {
				return expect(NodeID3.Promise.read(invalidFilepath)).to.eventually.be.rejectedWith(
					Error
				);
			});
		});
		describe("with valid file path", function () {
			...
			it("resolve", function () {
				return expect(NodeID3.Promise.read(testFilepath)).to.eventually.to.have.key("raw");
			});
			after(function () {
				fs.unlinkSync(testFilepath);
			});
	...
	describe("#removeTags()", function () {
		...
		describe("with valid file path", function () {
			...
			it("resolve", function () {
				return expect(NodeID3.Promise.removeTags(testFilepath)).to.eventually.be.fulfilled;
			});
			after(function () {
				fs.unlinkSync(testFilepath);
			});
