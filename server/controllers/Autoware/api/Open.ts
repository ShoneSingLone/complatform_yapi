...
function handleValue(val, global, records) {
...
async function handleTest(interfaceData, handleValue, records, getUid) {
  let requestParams = {};
  let options;
  options = handleParams(interfaceData, (val, global) => handleValue(val, global, records), requestParams);
  let result = {
    id: interfaceData.id,
  ...
  try {
    options.taskId = getUid();
    let data = await crossRequest(
      options,
      interfaceData.pre_script,
      interfaceData.after_script,
      createContex(getUid(), interfaceData.project_id, interfaceData.interface_id)
    );
    let res = data.res;
    ...
    await handleScriptTest(interfaceData, responseData, validRes, requestParams, records, getUid);
...
async function handleScriptTest(interfaceData, response, validRes, requestParams, records, getUid) {
  try {
      ...
      post: {
        ...
        request: {
          body: {
            ...
            dataSync: {
              description: "数据同步（已废弃，使用merge）",
              type: "string"
            }
          }
        },
        async handler(ctx) {
          let type = ctx.payload.type;
          let content = ctx.payload.json;
          let project_id = ctx.payload.project_id;
          let dataSync = ctx.payload.merge;
           ...
           */
          try {
            if (!dataSync && ctx.payload.dataSync) {
              warnMessage =
                "importData Api 已废弃 dataSync 传参，请联系管理员将 dataSync 改为 merge.";
              dataSync = ctx.payload.dataSync;
            }
          } catch (e) {}
          ...
          try {
            ...
            if (ctx.payload.url) {
              content = await syncGet(ctx.payload.url);
          ...
          let menuList = await orm.interfaceCategory.list(project_id);
          /**
           * 防止分类被都被删除时取不到 selectCatid
           * 如果没有分类,增加一个默认分类
           */
          if (menuList.length === 0) {
            const catInst = orm.interfaceCategory;
    ...
    "/open/run_auto_test": {
      get: {
        ...
        async handler(ctx) {
          ...
          const project_id = ctx.payload.project_id;
          const startTime = new Date().getTime();
          const records = {};
          const reports = {};
          const testList = [];
          let id = ctx.payload.id;
          let curEnvList = handleEvnParams(ctx.payload);
          ...
          for (let i = 0, l = caseList.length; i < l; i++) {
            ...
            item.req_headers = handleReqHeader(
              item.req_headers,
              projectEvn.env,
              item.case_env
            );
            ...
            try {
              result = await handleTest(item, handleValue, records, () => this.getUid());
            } catch (err) {
              result = err;
            }
        ...
        <p>${autoTestUrl}</p>
          ...
          let mode = ctx.payload.mode || "html";
          if (ctx.payload.download === true) {
            ctx.set("Content-Disposition", `attachment; filename=test.${mode}`);
          }
          if (ctx.payload.mode === "json") {
