...
class userController extends BaseController {
  ...
  async login(ctx) {
    ...
    } else if (xU.generatePassword(password, result.passsalt) === result.password) {
  ...
  async changePassword(ctx) {
    ...
    if (this.getRole() !== 'admin' || user.role === 'admin') {
      ...
      if (xU.generatePassword(params.old_password, user.passsalt) !== user.password) {
  ...
  async avatar(ctx) {
    try {
      ...
      if (!data || !data.basecode) {
        dataBuffer = xU.fs.readFileSync(path.join(xU.WEBROOT, 'static/image/avatar.png'));
        type = 'image/png';
      } else {
        type = data.type;
