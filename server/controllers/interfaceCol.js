...
class interfaceColController extends BaseController {
  ...
  async getCaseEnvList(ctx) {
    try {
      ...
      for (let i = 0; i < projectList.length; i++) {
        let result = await this.modelProject.getBaseInfo(projectList[i], 'name  env');
  ...
  async getCaseListByVariableParams(ctx) {
    try {
      ...
      let project = await this.modelProject.getBaseInfo(resultList[0].project_id);
  ...
  async upCase(ctx) {
    try {
      ...
      this.modelProject.up(caseData.project_id, { up_time: new Date().getTime() }).then();
  ...
  async getCase(ctx) {
    try {
      ...
      let data = await this.interfaceModel.get(result.interface_id);
      if (!data) {
        return (ctx.body = xU.resReturn(null, 400, '找不到对应的接口，请联系管理员'));
      }
      data = data.toObject();
