...
本项目是一个接口管理平台，主体框架基于 [xspace](https://github.com/YMFE/xspace)，融合了 DocFlow 文档流功能，为团队提供完
整的接口管理和文档协作解决方案。

!\[qq_group]\(./docs/qq_group.png null)
...
-   接口增删改查（`Interface.ts`），支持
    `application/json`、`multipart/form-data`、`x-www-form-urlencoded`、`application/xml` 等请求体类型
...
-   Swagger 自动同步插件（`xspace-plugin-swagger-auto-sync/`），支持定时从 URL 同步接口
...
-   基于 json-schema-faker，根据接口定义的 req_body_schema 和 res_body_schema 自动生成 Mock 数据
...
-   邮箱 + 验证码注册（`xspace-plugin-advanced-mock/` 中的发送验证码接口）
-   密码加密存储（bcrypt）
...
| 插件                             | 文件            | 支持格式                 |
| -------------------------------- | --------------- | ------------------------ |
| xspace-plugin-import-postman     | `importData.js` | Postman Collection v1/v2 |
| xspace-plugin-import-swagger     | `importData.js` | Swagger 1.0/2.0 JSON     |
| xspace-plugin-import-har         | `importData.js` | HAR (HTTP Archive)       |
| xspace-plugin-import-xspace-json | `importData.js` | xspace JSON 导出格式       |
...
| 插件                               | 文件            | 输出格式               |
| ---------------------------------- | --------------- | ---------------------- |
| xspace-plugin-export-data          | `exportData.js` | Markdown               |
| xspace-plugin-export-data          | `exportData.js` | JSON                   |
| xspace-plugin-export-data          | `exportData.js` | HTML                   |
| xspace-plugin-export-swagger2-data | `exportData.js` | Swagger 2.0 YAML/JSON  |
| xspace-plugin-gen-services         | `exportData.js` | TypeScript/JS 服务代码 |
...
-   Wiki 插件（`xspace-plugin-wiki/`）扩展功能
...
| 插件名称                           | 功能                                     |
| ---------------------------------- | ---------------------------------------- |
| xspace-plugin-advanced-mock        | 高级 Mock：自定义脚本、条件匹配、IP 过滤 |
| xspace-plugin-import-postman       | 导入 Postman Collection                  |
| xspace-plugin-import-swagger       | 导入 Swagger 文档                        |
| xspace-plugin-import-har           | 导入 HAR 格式                            |
| xspace-plugin-import-xspace-json   | 导入 xspace JSON                           |
| xspace-plugin-export-data          | 导出 Markdown/JSON/HTML                  |
| xspace-plugin-export-swagger2-data | 导出 Swagger 2.0                         |
| xspace-plugin-gen-services         | 生成服务代码（TypeScript/JS）            |
| xspace-plugin-swagger-auto-sync    | Swagger 自动同步                         |
| xspace-plugin-statistics           | 接口使用统计                             |
| xspace-plugin-test                 | 测试集与自动化测试                       |
| xspace-plugin-wiki                 | Wiki 文档系统                            |
...
├── docs/             # 项目文档和资源
├── exts/             # 插件扩展
│   ├── xspace-plugin-advanced-mock/      # 高级 Mock 插件
│   ├── xspace-plugin-export-data/       # 数据导出插件
│   ├── xspace-plugin-export-swagger2-data/  # Swagger 导出
│   ├── xspace-plugin-gen-services/      # 服务生成插件
│   ├── xspace-plugin-import-har/        # HAR 导入
│   ├── xspace-plugin-import-postman/    # Postman 导入
│   ├── xspace-plugin-import-swagger/     # Swagger 导入
│   ├── xspace-plugin-import-xspace-json/   # xspace JSON 导入
│   ├── xspace-plugin-statistics/        # 统计插件
│   ├── xspace-plugin-swagger-auto-sync/ # Swagger 自动同步
│   ├── xspace-plugin-test/               # 测试插件
│   └── xspace-plugin-wiki/              # Wiki 文档插件
├── server/           # 后端服务
...
-   **资源管理**：RESOURCE_ASSETS_REMOTE 重命名为 CLOUD_DISK_ROOT，为单个地址
-   **Autoware 路由系统**：新的模块化路由系统，替代原有 xspace 路由
