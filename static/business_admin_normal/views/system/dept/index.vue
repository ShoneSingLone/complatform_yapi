	...
	<div class="x-page-view">
		<xPageContent>
			...
			<div class="x-page-content-middle mt8 flex horizon">
				<xTableVir :columns="configsTable.columns" :data="configsTable.data.list" :expandedRowKeys.sync="configsTable.data.expandedRowKeys" rowKey="deptId" />
			</div>
		</xPageContent>
	</div>
</template>
									...
									children: [
										{
											label: i18n("修改"),
											disabled: () => isLevel0,
											isHide: () => !vm.$auth.hasPermiOr(["system:dept:edit"]),
											onClick: async () => {
												_.$openModal({
													title: i18n("修改部门"),
													url: "@/views/system/dept/dept.upsert.dialog.vue",
										...
										{
											label: i18n("删除"),
											isHide: () => !vm.$auth.hasPermiOr(["system:dept:remove"]),
											onClick: async () => {
												vm.doDelete(rowData);
											}
										}
		...
		methods: {
			...
			async getTableData() {
				try {
					...
					const { TREE } = _adminTools.handleTree({ data, id: "deptId" });
