...
export default async function ({ PRIVATE_GLOBAL }) {
	...
	return {
		...
		methods: {
			...
			columnResizerMouseup(event) {
				...
				if (
					...
					differWidth = cpt_columnMinWidth - currentResizingColumn._realTimeWidth;
				} else {
		...
		render(h) {
			...
			// 列调整器处理器类名
			const handlerClass = {};
			handlerClass[clsName("column-resizer-handler")] = true;
			handlerClass["active"] = isColumnResizerHover || isColumnResizing;
