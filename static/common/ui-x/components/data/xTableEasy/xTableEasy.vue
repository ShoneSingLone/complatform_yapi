	...
	return {
		...
		computed: {
			...
			hasFixedColumn() {
				return this.colgroups.some(
					x => x.fixed === COLUMN_FIXED_TYPE.LEFT || x.fixed === COLUMN_FIXED_TYPE.RIGHT
				);
			},
				...
				if (nextRowEl) {
					...
					if (keyCode === KEY_CODES.ARROW_UP) {
						...
						if (isVirtualScroll) {
							diff =
								headerTotalHeight -
								(trOffsetTop - (containerScrollTop - parentOffsetTop));
						} else {
            ...
            */
			...
			setVirtualPhantomHeight() {
				...
				if (this.virtualScrollPositions.length) {
					totalHeight =
						this.virtualScrollPositions[this.virtualScrollPositions.length - 1].bottom;
				}
			...
			bodyCellMousedown({ event, rowData, column }) {
				...
				if (isOperationColumn(colKey, colgroups)) {
					...
					const { startRowKey, endRowKey, startRowKeyIndex, endRowKeyIndex } =
						bodyIndicatorRowKeys;
			...
			headerCellMousedown({ event, column }) {
				...
				const { isGroupHeader, colgroups, headerIndicatorColKeys, cellSelectionData } =
					this;
			...
			headerCellMouseover({ event, column }) {
				const { colgroups, isGroupHeader, isHeaderCellMousedown, headerIndicatorColKeys } =
					this;
			...
			editCellByClick({ isDblclick, rowKey, colKey }) {
				const { editOption, isCellEditing, hasEditColumn, editingCell, isEditColumn } =
					this;
        ...
        */
			...
			[INSTANCE_METHODS.START_EDITING_CELL]({ rowKey, colKey, defaultValue }) {
				const { editOption, colgroups, rowKeyFieldName, editingCell, cellSelectionData } =
					this;
		...
		render(h) {
			...
			// 使用h函数构建虚拟DOM
			return h("div", tableRootProps, [
				h(VueDomResizeObserver, tableContainerWrapperProps, [
					h("div", tableContainerProps, [
						// virtual view phantom
						this.getVirtualViewPhantom(),
						// vue 实例类型，访问dom时需要通过$el属性访问
						h(VueDomResizeObserver, tableWrapperProps, [
							h("table", tableProps, [
								// colgroup
								h(Colgroup, {
									props: {
								...
								// table header
								showHeader ? h(Header, headerProps) : null,
								// table body
								h(Body, bodyProps),
								// table footer
								h(Footer, footerProps)
							]),
							// cell selection
							enableCellSelection ? h(Selection, selectionProps) : null
						])
					]),
					// edit input
					enableCellSelection ? h(EditInput, editInputProps) : null,
					// contextmenu
					this.enableHeaderContextmenu || this.enableBodyContextmenu
						? h(VeContextmenu, contextmenuProps)
						: null,
					// column resizer
					enableColumnResize ? h(ColumnResizer, columnResizerProps) : null
				])
			]);
		}
	};
...
.ve-dropdown-popper
	...
	.ve-dropdown-items
	.ve-dropdown-items-warpper
	.ve-dropdown-items-li.active:hover {
	background-color: #e6f7ff;
}
...
.ve-dropdown-popper
	...
	.ve-dropdown-items
	.ve-dropdown-items-warpper
	.ve-dropdown-items-multiple:hover {
	background-color: #f3f3f3;
}
...
.ve-dropdown-popper
	...
	.ve-dropdown-operation-item:not(.ve-dropdown-filter-disable):hover {
	color: #108ee9;
}
...
.ve-table
	...
	thead.ve-table-header
	tr.ve-table-header-tr
	th.ve-table-header-th.ve-table-cell-indicator {
...
.ve-table
	...
	thead.ve-table-header
	tr.ve-table-header-tr
	th.ve-table-header-th.ve-table-cell-indicator-active {
...
.ve-table
	...
	thead.ve-table-header
	.ve-table-header-tr
	.ve-table-header-th.ve-table-fixed-right {
	position: sticky;
	z-index: 30;
}
...
.ve-table
	...
	thead.ve-table-header
	.ve-table-header-tr
	.ve-table-header-th.ve-table-sortable-column {
	cursor: pointer;
}
...
.ve-table
	...
	tbody.ve-table-body
	tr.ve-table-body-tr
	td.ve-table-operation-col.ve-table-cell-indicator,
.ve-table
	.ve-table-container
	.ve-table-content-wrapper
	table.ve-table-content
...
.ve-table
	...
	tbody.ve-table-body
	tr.ve-table-body-tr
	td.ve-table-operation-col.ve-table-cell-indicator-active,
.ve-table
	.ve-table-container
	.ve-table-content-wrapper
	table.ve-table-content
...
.ve-table
	...
	.ve-table-content-wrapper
	table.ve-table-content
	tbody.ve-table-body.ve-table-stripe
	tr.ve-table-body-tr:nth-child(2n + 1)
	td {
	background-color: #fafafa;
}
...
.ve-table
	...
	.ve-table-content-wrapper
	table.ve-table-content
	tbody.ve-table-body.ve-table-row-highlight
	tr.ve-table-body-tr.ve-table-tr-highlight
	td {
...
.ve-table
	...
	tbody.ve-table-body
	.ve-table-body-tr
	.ve-table-body-td.ve-table-fixed-left,
.ve-table
	.ve-table-container
	.ve-table-content-wrapper
	table.ve-table-content
	tbody.ve-table-body
	.ve-table-body-tr
	.ve-table-body-td.ve-table-fixed-right {
	position: sticky;
	z-index: 10;
}
...
.ve-table
	...
	.ve-table-content-wrapper
	table.ve-table-content
	tfoot.ve-table-footer.ve-table-fixed-footer
	tr
	td {
	position: sticky;
	z-index: 20;
...
.ve-table
	...
	tfoot.ve-table-footer
	.ve-table-footer-tr
	.ve-table-footer-td.ve-table-fixed-left,
.ve-table
	.ve-table-container
	.ve-table-content-wrapper
	table.ve-table-content
	tfoot.ve-table-footer
	.ve-table-footer-tr
	.ve-table-footer-td.ve-table-fixed-right {
	position: sticky;
	z-index: 30;
}
...
.ve-table
	.ve-table-container.ve-table-container-right-scrolling
