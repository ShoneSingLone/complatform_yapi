		...
		contextmenus.forEach(contextmenu => {
			...
			if (contentmenuCollectionItem) {
				...
				// left fixed column to
					...
					contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.LEFT_FIXED_COLUMN_TO
				) {
					//
					if (isOperationCol) {
				...
				// right fixed column to
					...
					contentmenuCollectionItem.type === CONTEXTMENU_NODE_TYPES.RIGHT_FIXED_COLUMN_TO
				) {
					//
					if (isOperationCol) {
						contentmenuCollectionItem.disabled = true;
...
export function setBodyContextmenuOptions({
	...
	if (enableBodyContextmenu) {
		...
		const rowCount =
			selectionRangeIndexes.endRowIndex - selectionRangeIndexes.startRowIndex + 1;
		const colCount =
			selectionRangeIndexes.endColIndex - selectionRangeIndexes.startColIndex + 1;
			...
			for (
				...
				for (
					...
					tableData[rowIndex][fieldName] =
						cellSelectionTableData[cellSelectionTableDataRowIndex][fieldName];
				}
