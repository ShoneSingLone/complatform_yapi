...
<template>
	<div class="height100 flex vertical ProjectInterfaceSectionInterfaceList">
					...
					<xBlock class="mt">
						<xForm>
							...
							<div class="flex end width100" span="2">
								...
								<xBtn preset="primary" @click="inject_project_interface_section.configsTable.onQuery({ page: 1 })">查询</xBtn>
							</div>
						</xForm>
					</xBlock>
					<template #collapse>
		...
		<div class="x-page-content-middle mt8 flex horizon">
			<xTableVir :columns="inject_project_interface_section.configsTable.columns" :data="inject_project_interface_section.configsTable.data.list" />
		</div>
	</div>
</template>
<script lang="ts">
