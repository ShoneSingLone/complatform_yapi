<template>
	<div id="ProjectHoppscotch" class="height100 width100 flex flex-col">
		...
		<div class="toolbar flex items-center p-3 bg-white border-b shadow-sm">
			<!-- 方法选择 -->
			<select v-model="selectedMethod" class="mr-2 p-2 border rounded bg-white hover:border-blue-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
			...
			<div class="flex1 flex flex-col overflow-hidden bg-white border-r">
				...
				<div class="tabs flex border-b bg-gray-50">
					<button 
						v-for="tab in tabs" 
						:key="tab.id"
						:class="['tab-button px-4 py-2 cursor-pointer text-sm font-medium', activeTab === tab.id ? 'border-b-2 border-blue-500 text-blue-600 bg-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100']"
				...
				<div class="tab-content flex1 overflow-auto p-4">
					...
					<div v-if="activeTab === 'body'" class="space-y-4">
						...
						<div class="flex items-center space-x-2">
							...
							<select v-model="contentType" class="p-2 border rounded text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
						...
						<textarea
							v-model="requestBody"
							placeholder="输入请求体"
							class="width100 height-[calc(100%-100px)] border rounded p-2 text-sm font-mono focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
						></textarea>
					</div>
					...
					<div v-if="activeTab === 'authorization'" class="space-y-4">
						...
						<div class="space-y-3">
							<div class="flex items-center space-x-2">
								...
								<select v-model="authType" class="p-2 border rounded text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
					...
					<div v-if="activeTab === 'pre-request'" class="space-y-4">
						...
						<textarea
							...
							placeholder="输入请求前脚本"
							class="width100 height-[calc(100%-50px)] border rounded p-2 text-sm font-mono focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
						></textarea>
					</div>
			...
			<div class="w-1/2 flex flex-col overflow-hidden bg-white">
				...
				<div class="p-3 border-b bg-gray-50 flex items-center justify-between">
					<h3 class="text-sm font-semibold text-gray-700 flex items-center">
						响应
						<span 
							:class="['ml-2 px-2 py-0.5 rounded text-xs font-medium', getStatusClass(responseStatus)]"
						>
							{{ responseStatus }}
						</span>
					</h3>
				...
				<div class="response-tabs flex border-b bg-gray-50">
					<button 
						...
						:class="['tab-button px-4 py-2 cursor-pointer text-xs font-medium', activeResponseTab === 'body' ? 'border-b-2 border-blue-500 text-blue-600 bg-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100']"
					>
						Body
					</button>
					<button 
						@click="activeResponseTab = 'headers'" 
						:class="['tab-button px-4 py-2 cursor-pointer text-xs font-medium', activeResponseTab === 'headers' ? 'border-b-2 border-blue-500 text-blue-600 bg-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100']"
					>
						Headers
					</button>
					<button 
						@click="activeResponseTab = 'cookies'" 
						:class="['tab-button px-4 py-2 cursor-pointer text-xs font-medium', activeResponseTab === 'cookies' ? 'border-b-2 border-blue-500 text-blue-600 bg-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100']"
					>
						Cookies
					</button>
				</div>
				...
				<div class="response-content flex1 overflow-auto p-4">
					...
					<div v-if="activeResponseTab === 'body'" class="space-y-2">
						...
						<pre class="width100 height100 bg-gray-50 p-3 rounded overflow-auto font-mono text-xs">{{ formattedResponse }}</pre>
					</div>
					...
					<div v-if="activeResponseTab === 'headers'" class="space-y-2">
						<div class="grid grid-cols-2 gap-2 text-xs">
							...
							>
								<span class="font-semibold text-gray-700">{{ key }}:</span> {{ value }}
							</div>
						</div>
					</div>
...
export default async function () {
	return defineComponent({
		...
		methods: {
			...
			// 构建完整请求头（包含授权信息）
			buildHeaders() {
				const headers = this.headers.filter(h => h.key).reduce((acc, h) => {
					acc[h.key] = h.value;
					return acc;
				}, {});
