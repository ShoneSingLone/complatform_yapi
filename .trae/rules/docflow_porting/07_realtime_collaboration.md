...
**实时协作编辑**是 DocFlow 的核心功能之一，允许多个用户同时编辑同一文档，实时同步编辑内容，提高团队协作效率。主要特点包
括：
...
class YjsClient {
	...
	// 初始化
	init(options = {}) {
		const { documentId, editor, url = "ws://localhost:1234" } = options;
		...
		// 创建 WebSocket 提供者
		this.provider = new window.Y.WebSocketProvider(url, documentId, this.ydoc);
...
<template>
	<div class="user-list-container">
		...
		<div class="user-list">
			<div v-for="user in users" :key="user.id" class="user-item" :class="{ active: user.id === currentUserId }">
...
export default async function ({ PRIVATE_GLOBAL }) {
	...
	// 动态加载 Yjs 依赖
	await _.$importVue(["/common/yjs/y.js", "/common/yjs/y-websocket.js"]);
...
router.put("/documents/:id", async ctx => {
	try {
		const { title, content } = ctx.request.body;
		const document = await DocFlowDocument.findByIdAndUpdate(ctx.params.id, { title, content }, { new: true });
		if (!document) {
			ctx.body = {
				success: false,
				error: "文档不存在"
...
2. **架构设计**：采用前后端分离架构，前端使用 Vue + Tiptap + Yjs，后端使用 Node.js + Hocuspocus + MongoDB，实现了完整的
   实时协作流程。
