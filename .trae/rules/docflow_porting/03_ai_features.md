...
export default async function ({ PRIVATE_GLOBAL }) {
  // 动态加载提示词模板
  const [brainstormingPrompt] = await _.$importVue([
    '@/utils/ai/prompts/brainstorming.vue'
  ]);
...
export default async function ({ PRIVATE_GLOBAL }) {
  // 动态加载提示词模板
  const [textPolishPrompt] = await _.$importVue([
    '@/utils/ai/prompts/text-polish.vue'
  ]);
...
export default async function ({ PRIVATE_GLOBAL }) {
  // 动态加载提示词模板
  const [contentCompletePrompt] = await _.$importVue([
    '@/utils/ai/prompts/content-complete.vue'
  ]);
...
<template>
  <div class="ai-toolbar">
    <button 
      ...
      @click="openBrainstorming" 
      title="头脑风暴"
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <path d="M8 2L11 5V11L8 14L5 11V5L8 2Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <button 
      class="ai-tool-button" 
      @click="openTextPolish" 
      title="文本润色"
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <path d="M4 10L7 13L12 8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M11 3H5C3.89543 3 3 3.89543 3 5V11C3 12.1046 3.89543 13 5 13H11C12.1046 13 13 12.1046 13 11V7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
      </svg>
      <span>文本润色</span>
    </button>
...
export default async function ({ PRIVATE_GLOBAL }) {
  // 动态加载 AI 组件
  const [AIMenu] = await _.$importVue([
    ...
    <div class="ai-menu-content">
      ...
      <div v-if="type === 'brainstorming'" class="ai-config-section">
        ...
        <div class="form-group">
          ...
          <select 
            id="brainstorming-creativity" 
            v-model="config.creativityLevel" 
            class="form-select"
          >
    ...
    return {
      ...
      mockStream(url, data, callbacks = {}) {
        ...
        const responses = [
          ...
          { type: 'data', content: '润色后：\n"这是一段经过精心润色的专业文本，表达更加流畅自然，能够有效传达核心信息。"\n\n' },
          { type: 'data', content: '修改说明：\n- 增强了文本的专业性\n- 优化了表达流畅度\n- 保持了原有的核心信息' },
...
<template>
  <div class="tiptap-editor-with-ai">
    <!-- 编辑器 -->
    <component :is="tiptapEditorComponent" 
               ref="editor"
    ...
    <!-- AI 工具栏 -->
    <component :is="aiToolbarComponent" 
               ref="aiToolbar"
               @open-ai-menu="openAIMenu"
    />
    
    <!-- AI 菜单 -->
    <component :is="aiMenuComponent" 
               v-if="showAIMenu"
               :type="currentAIMenu"
               :visible="showAIMenu"
...
通过移植 DocFlow 的 AI 功能，我们将为 business_yapi 项目带来强大的智能辅助能力，包括头脑风暴、文本润色、内容续写等功能。实施过程中，我们将严格遵循项目现有的前端代码规则，使用 `_.$importVue()` 动态加载机制，确保与现有架构的一致性和兼容性。
...
这些功能将显著提升 business_yapi 项目的编辑体验和内容创作效率，使其具备与现代 AI 辅助编辑工具相媲美的能力。